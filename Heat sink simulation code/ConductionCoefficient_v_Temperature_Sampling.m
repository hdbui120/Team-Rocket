function k = ConductionCoefficient_v_Temperature_Sampling(Tworking)
...
Cp_Table = [9.2, 12.6, 16.6, 19.8, 22.6, 25.4, 28.0, 31.7]; % J/(kg*K)

if Tworking >= temp(length(temp))
    B = [find((temp < Tworking),2,"last"),find((temp > Tworking),1,"last")];
    k = Cp_Table(B(1)) + (Cp_Table(B(2))-Cp_Table(B(1)))*(Tworking-temp(B(1)))/(temp(B(2))-temp(B(1)));
else
